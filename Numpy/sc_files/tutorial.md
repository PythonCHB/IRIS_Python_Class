...
np.array. You can also explicitly specify the data-type if the
...
numpy has a `Matrix` class, but it's not generally used -- it gets too complicated to handle the transition form arrays to matrixes.

Rather, you can do linear algebra operations with the 2-d arrays
